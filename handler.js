...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        const isROwner = [conn.decodeJid(global.conn.this.id), ...global.owner.map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
conn: this,
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                const comandosPermitidos = ['serbot', 'bots', 'kick', 'code', 'setprimary', 'n', 'grupo abrir', 'grupo cerrar', 'update', 'delsession', 'on', 'off'];
