...
let handler = async (m, { conn, args }) => {
 if (!args || !args[0]) return conn.reply(m.chat, '*\`Ingresa El link Del audio a descargar ❤️‍🔥\`*', m, fake)
await m.react('🕓');
//  if (!args[0]) return m.reply('*\`Ingresa El link Del vídeo a descargar ❤️‍🔥\`*');
