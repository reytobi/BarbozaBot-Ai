...
let rtx2 = "*⪛✰ ↫ Barboza  -  Bσƚ ↬ ✰⪜*\n\n✐ Cσɳҽxισɳ SυႦ-Bσƚ Mσԃҽ Cσԃҽ\n\n✰ Usa este Código para convertirte en un *Sub-Bot* Temporal.\n\n\`1\` » Haga clic en los tres puntos en la esquina superior derecha\n\n\`2\` » Toque dispositivos vinculados\n\n\`3\` » Selecciona Vincular con el número de teléfono\n\n\`4\` » Escriba el Código para iniciar sesion con el bot\n\n✧ No es recomendable usar tu cuenta principal."

// Define serbot and serbot2 here
let serbot = "Sorry, you need to wait a moment."; // Replace with your desired message
let serbot2 = "No slots available for Sub-Bots."; // Replace with your desired message

const __filename = fileURLToPath(import.meta.url)
const __dirname = path.dirname(__filename)
const BarbozaJBOptions = {}
...
BarbozaJBOptions.pathBarbozaJadiBot = pathBarbozaJadiBot
...
const pathCreds = path.join(pathBarbozaJadiBot, "creds.json")
if (!fs.existsSync(pathBarbozaJadiBot)){
fs.mkdirSync(pathBarbozaJadiBot, { recursive: true })}
try {
...
const { state, saveState, saveCreds } = await useMultiFileAuthState(pathBarbozaJadiBot)
...
msgRetry,
msgRetryCache,
browser: mcode ? ['Ubuntu', 'Chrome', '110.0.5585.95'] : ['Barboza-Suou (Sub Bot)', 'Chrome','2.0.0'],
version: version,
...
try {
if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(pathBarbozaJadiBot)}@s.whatsapp.net`, {text : '*HEMOS DETECTADO UNA NUEVA SESIÓN, BORRE LA NUEVA SESIÓN PARA CONTINUAR*\n\n> *SI HAY ALGÚN PROBLEMA VUELVA A CONECTARSE*' }, { quoted: m || null }) : ""
} catch (error) {
console.error(chalk.bold.yellow(`Error 440 no se pudo enviar mensaje a: +${path.basename(pathBarbozaJadiBot)}`))
}}
...
try {
if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(pathBarbozaJadiBot)}@s.whatsapp.net`, {text : '*SESIÓN PENDIENTE*\n\n> *INTENTÉ NUEVAMENTE VOLVER A SER SUB-BOT*' }, { quoted: m || null }) : ""
} catch (error) {
console.error(chalk.bold.yellow(`Error 405 no se pudo enviar mensaje a: +${path.basename(pathBarbozaJadiBot)}`))
}
...
if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(pathBarbozaJadiBot)}@s.whatsapp.net`, {text : '*CONEXIÓN PÉRDIDA*\n\n> *INTENTÉ MANUALMENTE VOLVER A SER SUB-BOT*' }, { quoted: m || null }) : ""
