...
const handler = async (m, { conn, text }) => {
    try {
        ...
        if (data.length === 0) {
            await conn.sendMessage(m.chat, { text: `❌ No se encontraron imágenes para "${text}".` }, { quoted: m,rcanal });
            return;
        }
        ...
        await conn.sendMessage(m.chat, { text: '❌ Ocurrió un error al intentar obtener la imagen. Inténtalo nuevamente.' }, { quoted: m,rcanal });
    }
};
