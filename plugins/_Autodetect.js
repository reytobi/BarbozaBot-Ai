...
export async function before(m, { conn, participants }) {
  ...
  // Asegúrate de que global.baileys esté definido antes de intentar importar
  if (global.baileys) {
    try {
      WAMessageStubType = (await import(global.baileys)).default;
    } catch (error) {
      console.error("Error al importar 'baileys':", error);
    }
  } else {
    console.error("global.baileys no está definido.");
    return; // Salir de la función si no se puede importar
  }
  ...
  if (chat.detect && m.messageStubType == 2) {
    ...
    await this.sendMessage(m.chat, { text: `📌 𝐀𝐇𝐎𝐑𝐀 *${m.messageStubParameters[0] == 'on' ? '𝐒𝐎𝐋𝐎 𝐀𝐃𝐌𝐈𝐍𝐒' : '𝐓𝐎𝐃𝐎𝐒'}* 𝐏𝐔𝐄𝐃𝐄𝐍 𝐄𝐃𝐈𝐓𝐀𝐑 𝐋𝐀 𝐈𝐍𝐅𝐎𝐑𝐌𝐀𝐂𝐈Ó𝐍 𝐃𝐄𝐋 𝐆𝐑𝐔𝐏𝐎`, mentions: [m.sender] }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100 });
